...
import (
	...
	omni "github.com/voxgig-sdk/crun-api-overview-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.CrunApiOverviewError); is {
				out := map[string]any{
					"name":    "CrunApiOverviewError",
...
func omniCtx(arg any, client *sdk.CrunApiOverviewSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.CrunApiOverviewSDK); is {
			client = live
