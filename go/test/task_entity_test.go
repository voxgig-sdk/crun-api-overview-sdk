...
func TestTaskEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set CRUN_API_OVERVIEW_TEST_TASK_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		taskRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath("existing.task", setup.data)))
		var taskRef01Data map[string]any
		if len(taskRef01DataRaw) > 0 {
			taskRef01Data = core.ToMapAny(taskRef01DataRaw[0][1])
		}
...
func taskBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("CRUN_API_OVERVIEW_TEST_TASK_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CRUN_API_OVERVIEW_TEST_TASK_ENTID"])
	if idmapResolved == nil {
		idmapResolved = core.ToMapAny(idmap)
	}

	if env["CRUN_API_OVERVIEW_TEST_LIVE"] == "TRUE" {
