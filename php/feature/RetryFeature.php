...
// CrunApiOverview SDK retry feature
...
class CrunApiOverviewRetryFeature extends CrunApiOverviewBaseFeature
{
    ...
    public function init(CrunApiOverviewContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (CrunApiOverviewContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(CrunApiOverviewContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(CrunApiOverviewContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
