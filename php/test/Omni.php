...
//    only - a typed CrunApiOverviewContext there would make every ctx
...
final class CrunApiOverviewOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new CrunApiOverviewContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new CrunApiOverviewSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new CrunApiOverviewResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new CrunApiOverviewError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new CrunApiOverviewResponse($clean['response']);
    ...
    public static function err_from_map($m): ?CrunApiOverviewError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new CrunApiOverviewError(is_string($code) ? $code : '', $msg);
    }
