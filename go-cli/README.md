# crun-api-overview-cli

boru-driven command-line client **and** interactive REPL for the CrunApiOverview
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/crun-api-overview-cli)
make build

# 2. See usage (words, entities, env vars)
./crun-api-overview-cli --help
...
CRUN_API_OVERVIEW_BASE=https://api.example.com ./crun-api-overview-cli --help

# 6. No arguments -> interactive REPL
./crun-api-overview-cli
crun-api-overview> /help
crun-api-overview> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/crun-api-overview-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/crun-api-overview-cli --help
   ```
...
Run with no arguments to open a REPL (prompt `crun-api-overview>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `generate`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/crun-api-overview-cli`. |
...
(`./crun-api-overview-cli <expr>`) and interactively (the REPL), and expressions compose the
