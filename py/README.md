# CrunApiOverview Python SDK



The Python SDK for the CrunApiOverview API — an entity-oriented client following Pythonic conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Generate()` — each
...
release tag (`py/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/crun-api-overview-sdk/releases)) or
...
client = CrunApiOverviewSDK({
    "apikey": os.environ.get("CRUN_API_OVERVIEW_APIKEY"),
})
```
...
| --- | --- | --- |
| `aspect_ratio` | `str` | Aspect ratio of the video (e.g., 16:9, 9:16, 1:1) |
| `callback_url` | `str` | Optional webhook URL to receive task completion notification |
| `duration` | `float` | Duration of the video in seconds |
| `height` | `int` | Height of the generated image in pixels |
| `image_url` | `str` | Optional reference image URL for image-to-video generation |
| `model` | `str` | Name of the image generation model to use (e.g., google-seedream, flux, qwen, z-image, wan, openai, grok, imagine) |
| `negative_prompt` | `str` | Text description of what to avoid in the generated image |
...
| Field | Type | Description |
| --- | --- | --- |
| `completed_at` | `str` | Timestamp when the task was completed (if applicable) |
...
```
py/
├── crunapioverview_sdk.py         -- Main SDK module
