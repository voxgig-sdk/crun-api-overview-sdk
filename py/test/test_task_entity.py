    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set CRUN_API_OVERVIEW_TEST_TASK_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        task_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.task")))
        task_ref01_data = None
        if len(task_ref01_data_raw) > 0:
...
def _task_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("CRUN_API_OVERVIEW_TEST_TASK_ENTID"))
    ...
    if env.get("CRUN_API_OVERVIEW_TEST_LIVE") == "TRUE":
        ...
        client = CrunApiOverviewSDK(helpers.to_map(merged_opts))

    _live = env.get("CRUN_API_OVERVIEW_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("CRUN_API_OVERVIEW_TEST_EXPLAIN") == "TRUE",
        "live": _live,
