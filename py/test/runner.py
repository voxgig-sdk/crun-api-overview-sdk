# CrunApiOverview SDK test runner
...
class CrunApiOverviewTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            CrunApiOverviewTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = CrunApiOverviewTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = CrunApiOverviewTestRunner.getenv("CRUN_API_OVERVIEW_TEST_LIVE")
        override = CrunApiOverviewTestRunner.getenv("CRUN_API_OVERVIEW_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = CrunApiOverviewTestRunner.getenv(key)
        ...
        explain = CrunApiOverviewTestRunner.getenv("CRUN_API_OVERVIEW_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if CrunApiOverviewTestRunner._test_control is not None:
            return CrunApiOverviewTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                CrunApiOverviewTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            CrunApiOverviewTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = CrunApiOverviewTestRunner.load_test_control()
...
def load_env_local():
    CrunApiOverviewTestRunner.load_env_local()


def env_override(m):
    return CrunApiOverviewTestRunner.env_override(m)


def entity_data(v):
    return CrunApiOverviewTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return CrunApiOverviewTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return CrunApiOverviewTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return CrunApiOverviewTestRunner.load_test_control()


def live_client_options():
    return CrunApiOverviewTestRunner.live_client_options()


def live_delay_ms():
    return CrunApiOverviewTestRunner.live_delay_ms()
