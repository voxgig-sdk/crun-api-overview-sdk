...
//     (\`new CrunApiOverviewSDK(...)\` / \`CrunApiOverviewSDK.test(...)\` become
//     \`CrunApiOverviewSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'CrunApiOverviewSDK'
...
const TEST_SEED = {"entity":{"generate":{"test01":{"id":"test01"}},"task":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new CrunApiOverviewSDK(...)\` and any
// \`CrunApiOverviewSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+CrunApiOverviewSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/CrunApiOverviewSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
