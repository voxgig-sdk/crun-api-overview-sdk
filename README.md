...
The Crun API provides access to a wide range of AI models for tasks such as image generation, video creation, and audio processing through a unified platform.

Learn more about Crun API at [docs.crun.ai](https://docs.crun.ai/).
...
This SDK exposes the API as a small set of **semantic entities** — Generate and Task — that you
...
```go
client := sdk.Test()
result, err := client.Task(nil).Load(
...
| --- | --- | --- |
| **Generate** | The Generate entity (create). | `/image/generate` |
| **Task** | The Task entity (load). | `/tasks/{task_id}` |
