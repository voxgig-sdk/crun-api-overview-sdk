...
class GenerateEntity
  ...
  def data_set(args)
    if args
      @_data = CrunApiOverviewHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = CrunApiOverviewHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #
  # @param reqdata [GenerateCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [Generate, Hash] the created Generate; raises CrunApiOverviewError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        if ctx.result.resdata
          @_data = CrunApiOverviewHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
