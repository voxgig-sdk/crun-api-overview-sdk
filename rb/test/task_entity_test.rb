...
class TaskEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set CRUNAPIOVERVIEW_TEST_TASK_ENTID JSON to run live"
      return
    end
    client = setup[:client]

    # Bootstrap entity data from existing test data.
    task_ref01_data_raw = Vs.items(Helpers.to_map(
      Vs.getpath(setup[:data], "existing.task")))
    task_ref01_data = nil
    if task_ref01_data_raw.length > 0
      task_ref01_data = Helpers.to_map(task_ref01_data_raw[0][1])
    end

    # LOAD
    task_ref01_ent = client.Task(nil)
    task_ref01_match_dt0 = {}
    task_ref01_data_dt0_loaded, err = task_ref01_ent.load(task_ref01_match_dt0, nil)
    assert_nil err
...
def task_basic_setup(extra)
  ...
  client = CrunApiOverviewSDK.test(options, extra)
  ...
  entid_env_raw = ENV["CRUNAPIOVERVIEW_TEST_TASK_ENTID"]
  ...
  idmap_resolved = Helpers.to_map(
    env["CRUNAPIOVERVIEW_TEST_TASK_ENTID"])
  ...
  if env["CRUNAPIOVERVIEW_TEST_LIVE"] == "TRUE"
    ...
    client = CrunApiOverviewSDK.new(Helpers.to_map(merged_opts))
  end
