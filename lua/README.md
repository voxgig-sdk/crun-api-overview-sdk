...
The Lua SDK for the CrunApiOverview API. Provides an entity-oriented interface using Lua conventions.
...
```lua
-- Create
local created, _ = client:Generate(nil):create({ name = "Example" }, nil)
...
local result, err = client:CrunApiOverview(nil):load(
  { id = "test01" }, nil
)
