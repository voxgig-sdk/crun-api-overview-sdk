...
require("crun-api-overview_types")
...
local CrunApiOverviewSDK = {}
CrunApiOverviewSDK.__index = CrunApiOverviewSDK
...
CrunApiOverviewSDK._make_feature = _make_feature


function CrunApiOverviewSDK.new(options)
  local self = setmetatable({}, CrunApiOverviewSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function CrunApiOverviewSDK:options_map()
...
function CrunApiOverviewSDK:get_utility()
  return Utility.copy(self._utility)
end


function CrunApiOverviewSDK:get_root_ctx()
  return self._rootctx
end


function CrunApiOverviewSDK:prepare(fetchargs)
...
function CrunApiOverviewSDK:direct(fetchargs)
...
function CrunApiOverviewSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function CrunApiOverviewSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "CrunApiOverviewSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function CrunApiOverviewSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function CrunApiOverviewSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "CrunApiOverviewSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function CrunApiOverviewSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = CrunApiOverviewSDK.new(sdkopts)
  sdk.mode = "test"
