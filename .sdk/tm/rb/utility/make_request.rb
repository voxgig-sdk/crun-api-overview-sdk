...
module CrunApiOverviewUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = CrunApiOverviewResponse.new({})
    ...
    if fetch_err
      ...
      response = CrunApiOverviewResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = CrunApiOverviewResponse.new(fetched)
    else
