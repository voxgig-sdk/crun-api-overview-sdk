...
// CrunApiOverview SDK feature test
//
...
class FtCtrl extends CrunApiOverviewControl
{
...
class FtClient
{
    ...
    private CrunApiOverviewUtility $utility;

    public function __construct(CrunApiOverviewUtility $utility, array $options)
    {
    ...
    public function get_utility(): CrunApiOverviewUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (CrunApiOverviewContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public CrunApiOverviewContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (CrunApiOverviewContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = CrunApiOverviewConfig::make_feature($name);
    ...
    private static function buildUrl(CrunApiOverviewSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof CrunApiOverviewError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new CrunApiOverviewResponse($fetched);
        }
    ...
    private function fail(CrunApiOverviewContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (CrunApiOverviewContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
