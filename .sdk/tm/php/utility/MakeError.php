...
class CrunApiOverviewMakeError
{
    public static function call(?CrunApiOverviewContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new CrunApiOverviewContext([], null);
        }
        $op = $ctx->op ?? new CrunApiOverviewOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new CrunApiOverviewResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof CrunApiOverviewError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new CrunApiOverviewError('', $msg, $ctx);
        ...
        if ($err instanceof CrunApiOverviewError) {
