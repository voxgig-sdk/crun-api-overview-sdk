# crun-api-overview-mcp

[MCP](https://modelcontextprotocol.io) server exposing the CrunApiOverview SDK as
two agent tools — `crun-api-overview_list` and `crun-api-overview_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/crun-api-overview-mcp)
make build

# 2. Provide credentials via the environment
export CRUN_API_OVERVIEW_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user crun-api-overview \
  -- /absolute/path/to/crun-api-overview-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./crun-api-overview-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// crun-api-overview_list: first page of records
{ "entity": "generate" }
{ "entity": "generate", "query": { } }

// crun-api-overview_load: one record by id
{ "entity": "task", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/crun-api-overview-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user crun-api-overview \
     -- "$PWD"/dist/*/crun-api-overview-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `crun-api-overview_list` and `crun-api-overview_load` tools now appear
   in new sessions. Ask the agent to *"list generate using crun-api-overview"*
   and it calls `crun-api-overview_list` with `{"entity":"generate"}`.
...
```sh
export CRUN_API_OVERVIEW_APIKEY=sk_live_xxx            # API key
export CRUN_API_OVERVIEW_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `crun-api-overview_list` tool
...
### Call the `crun-api-overview_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `crun-api-overview_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `crun-api-overview_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `CRUN_API_OVERVIEW_APIKEY` | API key sent with every request. |
| `CRUN_API_OVERVIEW_BASE` | Optional override of the API base URL. |
...
```sh
./crun-api-overview-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"crun-api-overview_load","arguments":{"entity":"task","query":{"id":1}}}}'
```
